...
export async function handleDirectoryWebhook(request, env, ctx) {
    try {
      ...
      if (!apiToken || !accountId) {
        return new Response(JSON.stringify({
          success: false,
          message: 'Missing Cloudflare API credentials in environment variables'
        }), {
          status: 500,
          headers: { 'Content-Type': 'application/json' }
        });
  ...
  function extractDirectoryFromPayload(payload) {
    ...
    if (payload.data) {
      ...
      // Extract from slug if available
      if (payload.data.slug || payload.data.Slug) {
        const slug = payload.data.slug || payload.data.Slug;
